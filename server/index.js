...
/**
 * Lista os itens disponíveis para compor cestas básicas.
 * PROTEGIDO - Requer autenticação
 */
app.get('/api/cestas/itens', authenticateToken, async (req, res) => {
  try {
...
/**
 * Lista o estoque atual por item. LEFT JOIN garante que itens sem linha
 * em ESTOQUE_ITENS retornem qtdEst = 0.
 * PROTEGIDO - Requer autenticação
 */
app.get('/api/cestas/estoque', authenticateToken, async (req, res) => {
  try {
    const { rows } = await pool.query(`
...
/**
 * Registra entrada de doação no estoque.
 * Incrementa ESTOQUE_ITENS atomicamente e grava linha de auditoria em ESTOQUE_MOV.
 * PROTEGIDO - Requer autenticação
 */
app.post('/api/cestas/estoque/entrada', authenticateToken, async (req, res) => {
 ...
 * Query: ?page=1&limit=20&beneficiario=<nome>&dataInicio=YYYY-MM-DD&dataFim=YYYY-MM-DD
 * PROTEGIDO - Requer autenticação
 */
app.get('/api/cestas', authenticateToken, async (req, res) => {
  try {
...
/**
 * Registra entrega de cesta para um beneficiário.
 * Decrementa estoque atomicamente e grava ESTOQUE_MOV (tipo='S') por item.
 * PROTEGIDO - Requer autenticação
 */
app.post('/api/cestas', authenticateToken, async (req, res) => {
...
/**
 * Detalha uma entrega de cesta (cabeçalho + itens).
 * PROTEGIDO - Requer autenticação
 */
app.get('/api/cestas/:id', authenticateToken, async (req, res) => {
...
/**
 * Atualiza uma entrega: estorna estoque dos itens antigos (MOV='E'),
 * deleta CESTA_ITENS antigos, valida estoque dos itens novos,
 * atualiza CESTAS, insere CESTA_ITENS novos e decrementa estoque (MOV='S').
 * PROTEGIDO - Requer autenticação
 */
app.put('/api/cestas/:id', authenticateToken, async (req, res) => {
  const id = parseInt(req.params.id, 10);
  if (!Number.isInteger(id) || id <= 0) {
    return res.status(400).json({ error: 'id inválido.' });
  }
